...
# ── Logging ───────────────────────────────────────────────────────────────────
def setup_logger(log_dir: str, name: str = "example1") -> logging.Logger:
    ...
    # Also log to stdout (plain, no rich markup)
    sh = logging.StreamHandler(sys.stdout)
    sh.setFormatter(logging.Formatter("%(message)s"))
    sh.setLevel(logging.WARNING)   # only warnings+ to stdout (rich handles INFO)
...
def print_banner():
    ...
    console.print(Panel.fit(
        Text.assemble(
            ("  Example 1 — Cat vs Dog CNN Classifier  \n", "bold white"),

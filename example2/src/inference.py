...
# Colour palette for annotation boxes (one per class index)
_CLASS_COLOURS = [
    (220, 50,  50),   # index 0 — red-ish
    (50,  180, 50),   # index 1 — green-ish
    (50,  120, 220),  # index 2 — blue-ish
    (220, 150, 30),   # index 3 — orange-ish
    (150, 50,  220),  # index 4 — purple-ish
]


def _class_colour(pred_idx: int) -> tuple:
    return _CLASS_COLOURS[pred_idx % len(_CLASS_COLOURS)]
...
def _annotate_and_save(image_path: str, pred_class: str, pred_idx: int,
                        confidence: float, out_dir: str, cfg_inf: dict):
    """Draw label box on image and save."""
    img     = Image.open(image_path).convert("RGB")
    draw    = ImageDraw.Draw(img)
    w, h    = img.size
    label   = f"{pred_class}  {confidence*100:.1f}%"
    colour  = _class_colour(pred_idx)
...
def run_inference(cfg: dict, model_path: str, input_path: str):
        ...
        for i in range(0, len(image_paths), batch_size):
            ...
            for j in range(len(batch_paths)):
                ...
                if inf_cfg.get("save_annotated_images", True) and r["pred_idx"] != -1:
                    _annotate_and_save(r["path"], r["pred_class"], r["pred_idx"],
    ...
    rt.add_column("Predicted",       style="bold white",  width=12)
    rt.add_column("Confidence",      style="yellow",      width=12, justify="right")
    for cn in class_names:
        rt.add_column(f"P({cn})",    style="dim",         width=12, justify="right")
    rt.add_column("Latency (ms)",    style="dim",         width=14, justify="right")
    if has_gt:
        rt.add_column("True Label",  style="cyan",        width=12)
    ...
    if has_gt:
        ...
        for cn in class_names:
            cm_t.add_column(cn, style="yellow", width=12, justify="center")
        for i, cn in enumerate(class_names):
